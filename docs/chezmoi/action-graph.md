...
| [`.chezmoiscripts/run_onchange_after_52-sync-wezterm-config.sh.tmpl`](../../.chezmoiscripts/run_onchange_after_52-sync-wezterm-config.sh.tmpl)     | after `52`, WSL2 only         | Delegate rendered WezTerm Windows sync to mise.                                    | Re-runs when rendered WSL2 delegation script content changes, including the WezTerm config hash.                                                       | `.is_wsl`, `CI`, `.paths.mise`, and rendered WezTerm config.                                           | Explicit `dot_config/wezterm/wezterm.lua.tmpl` hash comment.                          | Delegates to `mise run sync:wezterm`; the task copies only when the WSL-side source config exists.                                         | Missing mise binary, `sync:wezterm` failure, PowerShell, `wslpath`, directory creation, or copy failure on WSL2.                                                         | Skips in CI; no-op when source config does not exist; non-WSL hosts render no operation.                               | WSL2 local apply, rendered WezTerm inspection, `mise run sync:wezterm`, CI skip path.       |
